-- ═══════════════════════════════════════════════════════════════
-- MEBLI-PRO: ПОВНЕ ОНОВЛЕННЯ БАЗИ ДАНИХ
-- Виконайте цей файл в Supabase SQL Editor (одним запуском)
-- ═══════════════════════════════════════════════════════════════

-- ═══════════════════════════════════════════════════════════════
-- 1. ОНОВЛЕННЯ КЕШУ СХЕМИ (ВАЖЛИВО ДЛЯ ANALYTICS)
-- ═══════════════════════════════════════════════════════════════
NOTIFY pgrst, 'reload schema';

-- ═══════════════════════════════════════════════════════════════
-- 2. ТАБЛИЦЯ site_events — додати колонку path
-- ═══════════════════════════════════════════════════════════════
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Додано колонку path в site_events';
    ELSE
        RAISE NOTICE 'Колонка path вже існує';
    END IF;
END $$;

-- ═══════════════════════════════════════════════════════════════
-- 3. ТАБЛИЦЯ blog — SEO поля
-- ═══════════════════════════════════════════════════════════════
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- ═══════════════════════════════════════════════════════════════
-- 4. ТАБЛИЦЯ reviews — модерація та Google Auth
-- ═══════════════════════════════════════════════════════════════
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- ═══════════════════════════════════════════════════════════════
-- 5. ТАБЛИЦЯ gallery — alt_text для зображень
-- ═══════════════════════════════════════════════════════════════
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
-- ═══════════════════════════════════════════════════════════════
-- 6. ОНОВЛЕННЯ 9 ПОСЛУГ В site_settings (ПОВНІ ДАНІ)
-- ═══════════════════════════════════════════════════════════════

-- Спочатку видалимо старий запис якщо є проблеми
DELETE FROM site_settings WHERE key = 'site';

-- Вставляємо новий запис з 9 послугами
...
-- ═══════════════════════════════════════════════════════════════
-- 8. ОНОВЛЕННЯ КЕШУ ЩЕ РАЗ (ВАЖЛИВО!)
-- ═══════════════════════════════════════════════════════════════
NOTIFY pgrst, 'reload schema';
...
SELECT key, jsonb_array_length(value->'services') as services_count 
FROM site_settings WHERE key = 'site';
